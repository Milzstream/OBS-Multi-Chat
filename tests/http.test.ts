...
describe('profile URL allowlist and Windows open args', () => {
  it('accepts platform profile URLs and rejects everything else', () => {
    ...
    assert.equal(isSafeExternalUrl('https://dashboard.twitch.tv/u/ada/stream-manager'), true)
    assert.equal(isSafeExternalUrl('https://dashboard.twitch.tv/u/ada/stream'), true)
    assert.equal(isSafeExternalUrl('https://kick.com/dashboard/stream'), true)
