...
let twitchSocket;
const state = {
    accounts: ['Twitch', 'Kick', 'YouTube'].map((platform) => ({ platform: platform, connected: Boolean(tokens[platform]), live: false, viewers: 0, handle: tokens[platform]?.user || '' })),
...
app.post('/api/stream-info', async (request, response) => {
    ...
    const results = await Promise.all(['Twitch', 'Kick'].map((platform) => updateStreamInfo(platform, state.streamInfo)));
    broadcast();
...
app.post('/api/disconnect/:platform', (request, response) => {
    ...
    if (platform === 'Twitch' && twitchSocket)
        twitchSocket.close();
    broadcast();
...
function authorizationUrl(platform) {
    ...
    if (platform === 'Twitch')
        params.set('scope', 'user:read:email chat:read chat:edit channel:manage:broadcast');
    if (platform === 'Kick')
        params.set('scope', 'user:read channel:read chat:read chat:write');
    if (platform === 'YouTube') {
...
async function exchangeCode(platform, code) {
    ...
    if (!result.ok)
        throw new Error(`${platform} token exchange: ${result.status} ${await result.text()}`);
    ...
    if (platform === 'Twitch') {
        ...
        token.user = 'YouTube account';
    else
        token.user = 'Kick account';
    return token;
}
...
async function youtubeApi(endpoint, token) {
...
function connectTwitchChat() {
    if (!tokens.Twitch?.user || twitchSocket)
        return;
    ...
    twitchSocket.on('close', () => { twitchSocket = undefined; if (tokens.Twitch)
        setTimeout(connectTwitchChat, 5_000); });
...
function parseTwitchLines(raw) {
    return raw.split(/\r?\n/).flatMap((line) => {
        ...
        return [{ id: crypto.randomUUID(), platform: 'Twitch', user: tags['display-name'] || match[2], text: match[3], time: new Date().toISOString(), emotes: tags.emotes ? tags.emotes.split('/').map((item) => item.split(':')[0]) : [] }];
    });
}
function addMessage(message) { state.messages = [...state.messages.slice(-199), message]; broadcast(); }
async function pollLiveState() {
    try {
        ...
        console.error('Live state poll:', error);
    }
}
...
async function pollYouTube() {
    ...
    const broadcastItem = broadcasts.items?.find((item) => item.status?.lifeCycleStatus === 'live');
    ...
    for (const item of messages.items || [])
        if (!youtubeSeen.has(item.id)) {
            ...
            addMessage({ id: item.id, platform: 'YouTube', user: item.authorDetails?.displayName || 'YouTube user', text: item.snippet?.displayMessage || '', time: item.snippet?.publishedAt || new Date().toISOString() });
        }
}
...
async function sendMessage(platform, text) {
    ...
    if (platform === 'Twitch' && twitchSocket?.readyState === WebSocket.OPEN) {
        twitchSocket.send(`PRIVMSG #${tokens.Twitch?.user} :${text}\r\n`);
        return { platform, ok: true };
    }
    if (platform === 'Kick' && process.env.KICK_API_BASE)
        return { platform, ok: await kickRequest('/chat', { content: text }) };
    return { platform, ok: false, error: `${platform} chat is not live or its API adapter is unavailable` };
}
async function kickRequest(endpoint, body) { const response = await fetch(`${process.env.KICK_API_BASE}${endpoint}`, { method: 'POST', headers: { Authorization: `Bearer ${tokens.Kick?.accessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); return response.ok; }
async function updateStreamInfo(platform, info) {
    ...
    if (platform === 'Kick' && process.env.KICK_API_BASE)
        return { platform, ok: await kickRequest('/channels', info) };
    return { platform, ok: false, error: 'Configure KICK_API_BASE for the Kick adapter' };
}
function startAdapter(platform) { if (platform === 'Twitch')
    connectTwitchChat(); }
