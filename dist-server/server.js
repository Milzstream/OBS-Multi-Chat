...
function addMessage(message) {
    ...
    const mergeAt = state.messages.findIndex((item) => {
        if (item.text !== message.text)
            return false;
        if (Math.abs(Date.parse(item.time) - incomingTime) > 90_000)
            return false;
        const itemIsOwn = ownHandles().has(item.user.toLowerCase());
        const itemPlatforms = item.platforms || [item.platform];
        if (tracked && (item.id === tracked.id || (incomingIsOwn && itemIsOwn)))
            return true;
        if (!itemIsOwn || !incomingIsOwn)
            return false;
    ...
    if (mergeAt >= 0) {
        ...
        if (platformsUnchanged && parts === current.parts && !dualYouTube)
            return;
